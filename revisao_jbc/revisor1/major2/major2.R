...
dataSet <- read.csv("./revisao_jbc/revisor1/major2/dataset.csv", header=TRUE, sep=",", stringsAsFactors=FALSE)
...
resultadoBinomial = data.frame(quartile=character(), p=numeric(), X=numeric(), Fail=numeric(), n=numeric(),
...
for (idx in 1:nrow(dataSetAPCount)) {
  ...
  temp <- data.frame(quartile=NA, p=NA, X=NA, Fail=NA, n=NA,
  ...
  # X - APs do quartil
  temp$X = dataSetAPCount[idx,]$freq

  # Casos onde a condição falhou ou NonAP
  temp$Fail = dataSetNonAPCount[idx,]$freq

  # n - número totas de amostras
